			  	...
			  	<form action="{{ route('ortu.update',$ortu->id) }}" method="post" >
        			...
        			{{ csrf_field() }}
			  		<div class="form-group {{ $errors->has('nama') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" name="nama" value="{{ $ortu->nama }}" class="form-control"  required>
			  			@if ($errors->has('nama'))
                     ...
                     <div class="form-group {{ $errors->has('umur') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" name="umur" value="{{ $ortu->umur }}" class="form-control"  required>
			  			@if ($errors->has('umur'))
                            <span class="help-block">
			  		...
			  		<div class="form-group {{ $errors->has('pasien_id') ? ' has-error' : '' }}">
			  			...
			  			<select name="pasien_id" class="form-control">
			  				@foreach($psn as $data)
			  				<option value="{{ $data->id }}" {{ $selectedPsn == $data->id ? 'selected="selected"' : '' }} >{{ $data->nama }}</option>
			  				@endforeach
			  			</select>
